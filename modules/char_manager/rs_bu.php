...
if(isset($use_ipbonus) && $use_ipbonus == 1 && $_SESSION['ipbonus_active'] == 1) {
    ...
    $gc_rs_ipbonus = "<br />- <font color='red'>Giảm thêm <strong><font color='blue'>$ipbonus_decre_price%</font></strong> khi chơi tại <a href='#acc_manager&act=ipbonus_list' rel='ajax'><strong><font color='blue'>IP Bonus</font></strong></a></font>";
    $vc_rs_ipbonus = "<br />- <font color='red'>Giảm thêm <strong><font color='blue'>$ipbonus_decre_price%</font></strong> khi chơi tại <a href='#acc_manager&act=ipbonus_list' rel='ajax'><strong><font color='blue'>IP Bonus</font></strong></a></font>";
}
// IP Bonus End
...
if($use_gioihanrs[$thehe_choise] == 1) {
    ...
    $gcoin_phat = $gioihanrs_rs_bu[$thehe_choise][$ghrs_cap_max];
	for($i=1; $i<= $ghrs_cap_max; $i++) {
        if( ($char_in_top > $ghrs_top_cap[$i-1]) && ($char_in_top <= $ghrs_top_cap[$i]) ) {
            ...
            $gcoin_phat = $gioihanrs_rs_bu[$thehe_choise][$i];
            break;
        }
    }
    $vpoint_phat = floor($gcoin_phat*(1+$vpoint_extra/100));
    
    $gcoin_rs_bu = $gc_rs_after + $gcoin_phat;
    $vpoint_rs_bu = $vc_rs_after + $vpoint_phat;
    
    $chenhlech_gcoin = ($_SESSION['acc_gcoin'] + $_SESSION['acc_gcoin_km']) - $gcoin_rs_bu;
    $chenhlech_vpoint = $_SESSION['acc_vpoint'] - $vpoint_rs_bu;
	...
	if( $_SESSION['nv_resetday'] >= $gioihanrs ) 
	{
        ...
        if($_SESSION['nv_resetday'] >= ($gioihanrs+$overrs_rs[$thehe_choise])) {
        	$notice_vuotmuc = "$font_blue_begin <b> $overrs_rs[$thehe_choise] </b> / {$overrs_rs[$thehe_choise]} $font_blue_end"; 
        } else {
            $accept = 0;
        	$rs_overed = $_SESSION['nv_resetday'] - $gioihanrs;
        	$notice_vuotmuc = "$font_red_begin <b> $rs_overed </b> $font_red_end / $font_blue_begin {$overrs_rs[$thehe_choise]} $font_blue_end (Chưa RS hết hôm nay)";
        }
		$notice_resetday = "$font_blue_begin $_SESSION[nv_resetday] / $gioihanrs $font_blue_end";
	} else {
	   $accept = 0;
		$notice_vuotmuc = "$font_red_begin <b> 0 </b> $font_red_end / $font_blue_begin {$overrs_rs[$thehe_choise]} $font_blue_end (Chưa RS hết hôm nay)"; 
		$notice_resetday = "$font_red_begin $_SESSION[nv_resetday] $font_red_end / $font_blue_begin $gioihanrs $font_blue_end (Chưa RS hết hôm nay)";
	}
    
    if($dayofweek == 1) {
        $gioihanrs_yesterday = $gioihanrs + $overrs_rs[$thehe_choise] + floor($gioihanrs * $overrs_sun_extra[$thehe_choise]/100);
    } else if($dayofweek == 0) {
        $gioihanrs_yesterday = $gioihanrs + $overrs_rs[$thehe_choise] + floor($gioihanrs * $overrs_sat_extra[$thehe_choise]/100);
    } else {
        $gioihanrs_yesterday = $gioihanrs + $overrs_rs[$thehe_choise];
    }
    $rs_bu_total = $gioihanrs_yesterday - $_SESSION['nv_reset_yesterday'];
    $notifice_rs_yesterday = $_SESSION['nv_reset_yesterday'] ." / $gioihanrs_yesterday";
    
    if($_SESSION['nv_reset_yesterday'] + $_SESSION['nv_reset_bu_now'] < $gioihanrs_yesterday) {
        $notifier_rs_bu = "$font_du_begin ". $_SESSION['nv_reset_bu_now'] ." / $rs_bu_total $font_du_end";
    } else {
        $accept = 0;
        $notifier_rs_bu = "$font_thieu_begin ". $_SESSION['nv_reset_bu_now'] ." / $rs_bu_total $font_thieu_begin";
    }
	
} elseif($use_gioihanrs[$thehe_choise] == 2) {

    $gcoin_phat = $_SESSION['nv_gc_rs_bu'];
    $vpoint_phat = floor($gcoin_phat*(1+$vpoint_extra/100));
    ...
    $rs_max = $_SESSION['nv_rs_total_max'] + $gioihanrs+$overrs_rs[$thehe_choise];
    $rl_max = 0;
    foreach($rl_reset_relife as $k => $v) {
        if($rs_max > $v) {
            $rs_max = $rs_max - $v;
            $rl_max = $k;
        } else {
    ...
    for($relife_i = 0; $relife_i <= $_SESSION['nv_relife']; $relife_i++) {
        $rs_total_now += $rl_reset_relife[$relife_i];
    }
    $rs_total_now += $_SESSION['nv_reset'];
    
    if($_SESSION['nv_reset_yesterday'] + $_SESSION['nv_reset_bu_now'] < $gioihanrs_yesterday) {
        $notifier_rs_bu = "$font_blue_begin ". $_SESSION['nv_reset_bu_now'] ." / $rs_bu_total $font_blue_end";
    } else {
        $accept = 0;
        $notifier_rs_bu = "$font_red_begin ". $_SESSION['nv_reset_bu_now'] ." / $rs_bu_total $font_red_end";
    }
    
    if($rs_total_now >= $_SESSION['nv_rs_total_max']) {
        $accept = 0;
        $notifier_rs_max_end = "$font_red_begin Đã đạt $font_red_end";
    }
    $notifier_rs_max = "$font_blue_begin $rs_max $font_blue_end / $font_red_begin $rl_max $font_red_end";
...
$gc_rs_show = "<strong>". number_format($gcoin_rs_bu, 0, ',', '.') ."</strong>";
$gc_rs_show .= "<br />". number_format($gc_rs_after, 0, ',', '.');
if($gc_rs_after < $gcoin_reset_vip) $gc_rs_show .= " <font size=1>(<i><u>$gcoin_reset_vip</u></i>)</font>";
$gc_rs_show .= " phí RS + $gcoin_phat phí phạt.";
$gc_rs_decre = "$gc_rs_acc_vip $gc_rs_tanthu $gc_rs_fb $gc_rs_ipbonus";

$vc_rs_show = "<strong>". number_format($vpoint_rs_bu, 0, ',', '.') ."</strong>";
$vc_rs_show .= "<br />". number_format($vc_rs_after, 0, ',', '.');
if($vc_rs_after < $vpoint_reset_vip) $vc_rs_show .= " <font size=1>(<i><u>$vpoint_reset_vip</u></i>)</font>";
$vc_rs_show .= " phí RS + $vpoint_phat phí phạt.";
...
$page_template = "templates/char_manager/rs_bu.tpl";
?>