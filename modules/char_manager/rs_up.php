...
if($use_gioihanrs[$thehe_choise] == 1) {
    ...
    $gcoin_phat = $gioihanrs_up_rs_other[$thehe_choise][$ghrs_cap_max];
	for($i=1; $i<= $ghrs_cap_max; $i++) {
        if( ($char_in_top > $ghrs_top_cap[$i-1]) && ($char_in_top <= $ghrs_top_cap[$i]) ) {
            ...
            $gcoin_phat = $gioihanrs_up_rs_other[$thehe_choise][$i];
            break;
        }
    }
    $vp_phat = floor($gcoin_phat*(1+$vpoint_extra/100));
    ...
    $gcoin_phat = $_SESSION['nv_gc_rs_up'];
    $vp_phat = floor($gcoin_phat*(1+$vpoint_extra/100));
...
$gc_rs = $gc_rs_after + $gcoin_phat;
$vp_rs = floor($gc_rs*(1+$vpoint_extra/100));

$gc_rs_show = "<strong>". number_format($gc_rs, 0, ',', '.') ."</strong>";
...
$gc_rs_show .= " phí Reset + $gcoin_phat phí RS hộ";
$gc_rs_decre = "$gc_rs_acc_vip $gc_rs_tanthu $gc_rs_fb $gc_rs_ipbonus";

$vc_rs_show = "<strong>". number_format($vp_rs, 0, ',', '.') ."</strong>";
...
$vc_rs_show .= " phí Reset + $vp_phat phí RS hộ";
$vc_rs_decre = "$vc_rs_acc_vip $vc_rs_tanthu $vc_rs_fb $vc_rs_ipbonus";


$chenhlech_gcoin = ($_SESSION['acc_gcoin'] + $_SESSION['acc_gcoin_km']) - $gc_rs;
$chenhlech_vpoint = $_SESSION['acc_vpoint'] - $vp_rs;
...
if ( $_SESSION['nv_online'] == 1 || $chenhlech_zen < 0 || ( $chenhlech_gcoin < 0 && $chenhlech_vpoint < 0) ) { $accept = 0; }	

$page_template = "templates/char_manager/rs_up.tpl";
?>